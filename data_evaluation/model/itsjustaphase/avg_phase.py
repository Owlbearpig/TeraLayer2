...
f_slice = (0.9*THz < freqs) * (freqs < 0.96*THz)
freqs = freqs[f_slice]
...
plt.scatter(freqs, np.unwrap(ref_data[f_slice, 2])-diff, label=f"reference")
plt.scatter(freqs, np.unwrap(data_array[sam_idx, f_slice, 2]), label=f"sample idx: {sam_idx}")
sam_idx += 10
plt.scatter(freqs, np.unwrap(data_array[sam_idx, f_slice, 2]), label=f"sample idx: {sam_idx}")
plt.scatter(freqs, np.unwrap(np.mean(data_array[:, f_slice, 2], axis=0)), label=f"mean")
plt.xlabel("Frequency (Hz)")
...
plt.scatter(freqs, ref_data[f_slice, 2], label=f"reference")
plt.scatter(freqs, data_array[sam_idx, f_slice, 2], label=f"sample idx: {sam_idx}")
sam_idx += 10
plt.scatter(freqs, data_array[sam_idx, f_slice, 2], label=f"sample idx: {sam_idx}")
