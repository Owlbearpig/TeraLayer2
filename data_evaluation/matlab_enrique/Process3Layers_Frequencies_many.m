...
for k=1:200
    enes=floor(deltas(l)*rand(6,1)+500-deltas(l)/2);
...
subplot(2,1,1)
plot(deltas'*(f(2)-f(1))*1e-12,prom*1e6)
ylabel('Mean value ({\mu}m)')

subplot(2,1,2)
plot(deltas'*(f(2)-f(1))*1e-12,sdev*1e6)
xlabel('Total bandwidth (THz)')
ylabel('Std. deviation ({\mu}m)')
