...
ref_cw_fd, sam_cw_fd = load_cw_data(sam_idx=sam_idx, shift=1.5)
r_cw_fd = array([ref_cw_fd[:, 0].real, sam_cw_fd[:, 1] / ref_cw_fd[:, 1]]).T

ref_tds_td, sam_tds_td = load_tds_data(sam_idx=sam_idx, signal_shift=-5)
...
plt.plot(r_cw_fd[:, 0].real, np.roll(np.angle(r_cw_fd[:, 1]), -110), label=r"CW $\phi_{sam} - \phi_{ref}$ " + f"{sam_idx}")
plt.xlim((-0.1, 1.5))
