...
def main():
    ...
    plt.plot(tmm_td[:, 0], tmm_td[:, 1], label="tmm")
    plt.xlabel("Time (ps)")
    ...
    plt.plot(ref_fd[:, 0], 20 * np.log10(np.abs(r_tmm[:, 1] * ref_fd[:, 1])), label="tmm * ref")
    ...
    #plt.plot(ref_fd[:, 0], 10 * np.log10(np.abs(ref_fd[:, 1])), label="reference")
    #plt.plot(sam_fd[:, 0], 10 * np.log10(np.abs(sam_fd[:, 1])), label="sample")
    plt.plot(sam_fd[:, 0], np.angle(r_exp), label="r_exp")
    plt.plot(sam_fd[:, 0], phase_tmm, label="r_tmm")
    plt.xlabel("Frequency (THz)")
